...
function master_idle()
   ...
   -- フェーザーの照準表示
    ...
    if (cur_weapon ~= nil and cur_weapon.index ~= last_weapon) then
        if (last_weapon == _weapon_phaser ) then
	        Players[0].crosshairs.active = crosshairs_prefs;
        elseif (cur_weapon.index == _weapon_phaser ) then
	        Players[0].crosshairs.active = true;
        end
        last_weapon = cur_weapon.index;
    end 

    --フェーザー以外の照準設定を保持する
   
    if cur_weapon ~= nil and cur_weapon.index ~= _weapon_phaser  then
        crosshairs_prefs = Players[0].crosshairs.active;
    end
    ...
    for p in Players() do
        ...
        if ( p._poisoned >= 0) then
	        ...
	        if (p._poison_ticker <= 0) then
	            local dmg = interval_poison_damage - (tolerance_coeff * p._tolerance);
	            p:damage(dmg,  "suffocation");
	            local pt = p._poisoned/30;
...
function Triggers.player_damaged(victim_player, aggressor_player,       
    ...
    potions = victim_player.items[_item_epotion];
    if (life < 50) and (potions > 0) and (not railgun_in_action) then
        ...
        victim_player.items[_item_epotion] = potions - 1;
    end
   
    --[[ Process poison damage --]]
   
    if ( (damage_type == _damage_poison) and 
...
function Triggers.got_item(item, player)
    ...
    if (item == _item_health) then
        player:play_sound(229,1);
    end
    if (item == _item_antidote) then
