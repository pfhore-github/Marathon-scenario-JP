...
for k, t in pairs(gates) do
    enter_to[k] = function()
        Players[0]:position(t.x, t.y, t.z, t.dest);
...
function level_idle ()
    ...
    if crypt_activated and (not ghouls_activated) and (ghoul_timer <= 0) then
        for i = 1, # ghoul_polys do
            ...
            Monsters.new( gp.x, gp.y, 0, gp, _clone);
        end
        dead_ghouls = 0;
...
function respawn_mauvair()
    ...
    if (mauvair_lives > 1) then
        ...
        mauvair = Monsters.new(mp.x, mp.y, 0, mp, _master_wizard);
...
function level_monster_killed(victim, victor, projectile)
    ...
    mtype = victim.type;
    if (mtype == _master_wizard) then
        respawn_mauvair();
        if (mauvair_poly ) then
            Monsters.new(mauvair_poly.x, mauvair_poly.y, 0, mauvair_poly, _skull_snake );
        end
    end
    if (mtype == _clone) and door_closed then
