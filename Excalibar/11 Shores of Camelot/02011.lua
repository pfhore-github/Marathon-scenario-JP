...
function level_idle ()   
    ...
    if (not fireball) and Tags[10].active then
        fireball = true;
        Items.new(Polygons[327].x, Polygons[327].y, 0, 327, _item_wand);
        Items.new(Polygons[176].x, Polygons[176].y, 0, 176, _item_wand);
    ...
    if (not freed) and (rampart_pod.floor_height < 3.5) then
        ...
        if (not restoring_saved) then
            Monsters.new(9,-11, 0, 159, _white_knight);
            Monsters.new(-5.7, -18.3, 0, 557, _white_knight);
        end
    end
