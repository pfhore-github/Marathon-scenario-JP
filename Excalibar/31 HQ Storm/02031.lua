...
function level_idle ()
    ...
    if destruct_enabled and (not master_gat_dead) then
        master_gat = find_monster(_gat_captain);
        master_gat_dead = (master_gat == nil);
        gat_card = find_item(_item_sap_card);
        have_card = Players[0].items[_item_sap_card] > 0;
        if master_gat_dead and (gat_card == nil) and (not have_card) then
                Players[0]:print('GATの船長は倒れました。あなたは今彼のアクセスカードを持っています！');
            Players[0].items[_item_sap_card] = Players[0].items[_item_sap_card] + 1;
        end
    end
