...
function level_init (rs)
    ...
    has_tiny = Players[0].items[_item_tiny_module] > 0;
    ai_scan();
    ...
    if Players[0].items[_item_sap_card] <= 0 then
        Players[0].items[_item_sap_card] = 1;  --[[ if vidmaster jump, then give player the SAP card ]]
    end
    if has_tiny then
...
function ai_scan()
    ...
    for g in Monsters() do 
        mtype = g.type;
        mp = g.polygon;
        if (mtype == _white_knight) and (mp == cadet_start) then
    	    cadets_left = cadets_left + 1;
        elseif (mtype == _morgana) then
	        morgana = g;
        elseif (mtype == _master_wizard) then
    	    mauvair = g;
        end
    end
end
...
enter_to [release_switch] = function()
...
enter_to[tiny_redisplay] = function()
    if has_tiny and tiny_replay then
...
enter_to[ science_roof ] = function()
    if Lights[62].active then
        Players[0]:teleport(roof_top);
    end
end

enter_to[ science_teleporter[1] ] = function()
    if Lights[62].active then
        Players[0]:teleport(bldg_safety);
    end
end

enter_to[ science_teleporter[2] ] = enter_to[ science_teleporter[1] ] 

enter_to[ escape_panel[1] ] = function()
...
enter_to[ escape_panel[2] ] = enter_to[ escape_panel[1] ] 

enter_to[ siniestra_teleport] = function()
    if  meter_started then
...
local function telep1()
    ...
    if (sendback_timer == 30) or (sendback_timer == 60) or (sendback_timer == 90) then
        Players[0]:print(string.format(" %.0f ...", sendback_timer/30));
    end
    sendback_timer = sendback_timer - 1;
end

for _,k in ipairs( jahala_pod ) do
    enter_to[ k] = telep1
end
...
function level_idle ()
    ...
    if meter_started then
        ...
        if (meter_moving and (meter_cnt >= meter_movement)) then
            ...
            Players[0]:print(string.format('アクチニウムガスの濃度: %d %%', meter_value));
    ...
    if bubble_timer > 0 then
        ...
        if bubble_timer >= 64 and morg_poly ~= nil then
            morgie = Monsters.new(morg_x, morg_y, 0, morg_poly, _metal_bar);
            morgie.active = true;
    ...
    if fire_timer > 0 then
        ...
        if fire_timer > 70 and morg_poly ~= nil then
            morgie = Monsters.new(morg_x, morg_y, 0, morg_poly, _fire);
            morgie.active = true;
            fire_timer = 0;
        end
    end
...
function level_monster_killed(victim, victor, projectile)
    if victim then
        mtype = victim.type;
        mp = victim.polygon;
        if mtype == _morgana then
            morgana_bubble(victim, mp);
        elseif mtype == _master_wizard then
            Players[0]:print('モーベアは倒されました！');
            Players[0]:play_sound(233, 1);
            mauvair = nil;
        elseif (mtype == _future_bob) and (not meter_started) then
            Players[0]:print('ブリゲード戦士は倒されました！');
        elseif (mtype == _white_knight) and (mp.index ~= cadet_start) and (not meter_started) then
...
function release_cadets()
    ...
    for g in Monsters() do 
        mtype = g.type;
        mp = g.polygon;
        if (mtype == _white_knight) and (mp.index == cadet_start) then
	        found = found + 1;
    ...
    for i, v in ipairs(idx) do 
        ...
        local wk = Monsters.new(v.x, v.y, v.z, v.poly, _white_knight);
        wk.yaw = wk.facing;
    end
end
...
function level_projectile_detonated(type, owner, polygon, x, y, z)
    ...
    if ((type == _shot_plasma) and (not plasma_message) and 
 (owner == player) and (morgana and morgana.valid) ) then
    ...
    fire_x = { _shot_missile, _shot_lightning, _shot_fireball,
    _shot_biggrenade, _shot_photon
    }
    for _, v in ipairs(fire_x) do
        if (type == v) and has_tiny and (heli_stage == 0) and (polygon == roof_top) then
...
function level_cleanup()
    remove_items(_item_sword);  --[[ Next level switches to Lightsaber ]]
end
