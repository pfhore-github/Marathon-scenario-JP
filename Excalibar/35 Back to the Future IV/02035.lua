...
holodeck10_exit = 75;
_tiny_bot = _shadow_sorcerer;
tiny_bot = nil;
_marcel_bot = _magus_sorcerer;
marcel_bot = nil;
tiny_kills = 0;
marcel_kills = 0;
player_kills = 0;
...
function level_init (rs)
    Players[0].items[_item_tiny_module] = 1;
    ...
    tiny_bot_poly = Polygons[649];  --[[ Angle 90 degrees --]]
    marcel_bot_poly = Polygons[662];  --[[ Angle 90 degrees --]]
    ...
    remove_items(_item_sword, _item_axe);
end
...
function store_inventory(keep_item)
   ...
   items_to_restore = {};
    ...
    Players[0].items[_item_water_vial] = 0;
end
...
function launch_clone()
   ...
   local m = Monsters.new(clones[idx][3], clones[idx][4], 0, clones[idx][1], _clone);
   m.yaw = clones[idx][2];
end
...
function level_idle ()
    ...
    if (loc == start_clones) and (not clones_on) then
        clones_on = true;
        store_inventory(_item_railgun);
        tiny_bot = Monsters.new(tiny_bot_poly.x, tiny_bot_poly.y, 0, tiny_bot_poly, _tiny_bot);
        tiny_bot.yaw = 90;
        marcel_bot = Monsters.new(marcel_bot_poly.x, marcel_bot_poly.y, 0, marcel_bot_poly, _marcel_bot);
        marcel_bot.yaw = 90;
        tiny_kills = 0;
        marcel_kills = 0;
        player_kills = 0;
    ...
    if (loc == end_clones) and (clones_on) then
        ...
        Players[0]:print(string.format('プレーヤーの点数は = %d ', player_kills));
        Players[0]:print(string.format('マーセルの点数は = %d ', marcel_kills));
        Players[0]:print(string.format('タイニーの点数は = %d ', tiny_kills));
    ...
    if (loc == end_gallery) and (gallery_on) then
        ...
        Players[0]:print(string.format('合計発射数： %d ', gallery_shots));
        Players[0]:print(string.format('合計殺数： %d ', gallery_kills));
        if gallery_shots > 0 then
            ratio = math.floor(100 * gallery_kills / gallery_shots);
            Players[0]:print(string.format('命中率： %.0f %%', ratio));
        end
        Players[0]:print(string.format('合計点： %d ', gallery_score));
    end
...
function level_monster_killed(monster, player, shot)
    ...
    if ((mtype == _minor_dinobug) or (mtype == _major_dinobug)) and gallery_on then
        gallery_kills = gallery_kills + 1;
        gallery_score = gallery_score + 1;
        Players[0]:print('得点！  (１点)');
    end
    if (mtype == _minor_raptor) and gallery_on then
        gallery_kills = gallery_kills + 1;
        gallery_score = gallery_score + 5;
        Players[0]:print('得点！  (5 点)');
    end
    if (mtype == _major_raptor) and gallery_on then
        gallery_kills = gallery_kills + 1;
        gallery_score = gallery_score + 10;
        Players[0]:print('得点！  (10 点)');
    end
    if (mtype == _clone) and clones_on then
        owner = shot.owner;
        p = Players[0].monster;
        ...
        elseif owner == p then
            ...
            Players[0]:print(string.format('未知の加害者！ (%s)', owner));
        end
   end
   if (mtype == _tiny_bot) and (clones_on) then
      tiny_bot = Monsters.new(tiny_bot_poly.x, tiny_bot_poly.y, 0, tiny_bot_poly, _tiny_bot);
      tiny_bot.yaw = 90;
   end
   if (mtype == _marcel_bot) and (clones_on) then
      marcel_bot = Monsters.new(marcel_bot_poly.x, marcel_bot_poly.y, 0, marcel_bot_poly, _marcel_bot);
      marcel_bot.yaw = 90;
   end
end
...
function level_projectile_detonated(type, owner, polygon, x, y, z)
    p = Players[0].monster;
    if (type == _shot_snyper_bullet) and (owner ~= nil and owner.index == p.index) and gallery_on then
...
function level_got_item(item, player)
    if (item == _item_sword) then
        player.items[_item_vial] = player.items[_item_vial] + 200;
    end
end
