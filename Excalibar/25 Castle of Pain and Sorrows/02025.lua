...
stomp_min_damage = 30;
stomp_type = _damage_trex;
_shot_stomp = _shot_special;
stomp_term = 3;
hidden_alcove = 552;
gabriel_window = 998;
gabriel_poly = 480;
...
gabriel_kills = 0;
_gabriel1 = _cavebob;
_gabriel2 = _futurebob;
_weaker_knight = _major_gat;
...
function find_cleric()
 ...
 --[[ If player stepped into the Wand chamber, and the chamber is active, then give the player
      ...
      --]]
enter_to[stomp_poly] = function()
    if (not stomp_poly_active) then
        if has_wand then
        ...
        for i = 1, 10, 1 do
            if Players[0].items[_item_magic_scroll] > 0 then
                Players[0].items[_item_magic_scroll] = Players[0].items[_item_magic_scroll] - 1;
           end
        end
...
function level_idle ()
   ...
   get the stomp wand.
   --]]
    has_wand = Players[0].items[_item_wand] > 0;
    scroll_cnt = Players[0].items[_item_magic_scroll];
 ...
 (outer_window.active)) then
        gabriel1 = Monsters.new(gabriel_platform.polygon.x, gabriel_platform.polygon.y, 0, gabriel_platform.polygon, _gabriel1);
        gabriel1.yaw = 180;
...
--        camera:deactivate()
 ...
 (gabriel_lockdown)) then
    ...
    if gabriel_lockdown and (not gabriel_fight) and (fight_timer > fight_wait) then
        ...
        gabriel2 = Monsters.new(banquet_l1.x, banquet_l1.y, 0, banquet_l1, _gabriel2);
        gabriel2.yaw = 180;
      
        m = Monsters.new(banquet_l2.x, banquet_l2.y, 0, banquet_l2, _weaker_knight);
        m = Monsters.new(banquet_l3.x, banquet_l3.y, 0, banquet_l3, _weaker_knight);
        m = Monsters.new(banquet_l4.x, banquet_l4.y, 0, banquet_l4, _weaker_knight);
        w1 = Monsters.new(banquet_r1.x, banquet_r1.y, 0, banquet_r1, _weaker_knight);
        w2 = Monsters.new(banquet_r2.x, banquet_r2.y, 0, banquet_r2, _weaker_knight);
      
        m = Monsters.new(banquet_r3.x, banquet_r3.y, 0, banquet_r3, _weaker_knight);
        m = Monsters.new(banquet_r4.x, banquet_r4.y, 0, banquet_r4, _weaker_knight);
        m = Monsters.new(banquet_r5.x, banquet_r5.y, 0, banquet_r5, _lesser_knight);
        m = Monsters.new(banquet_r6.x, banquet_r6.y, 0, banquet_r6, _lesser_knight);
    end
   --[[ 
   ...
   --]]
    if gabriel_lockdown and gabriel_dead then
        ...
        gabriel_sequence = 0;
        fight_timer = 0;
        MonsterTypes[_black_knight].enemies["player"] = true;
        MonsterTypes[_black_knight].friends["player"] = false;
        ...
        m = Monsters.new(banquet_r1.x, banquet_r1.y, 0, banquet_r1, _black_knight);
        m = Monsters.new(banquet_r6.x, banquet_r6.y, 0, banquet_r6, _black_knight);
        m = Monsters.new(banquet_l2.x, banquet_l2.y, 0, banquet_l2, _novice_sorcerer);
        m = Monsters.new(banquet_l3.x, banquet_l3.y, 0, banquet_l3, _novice_sorcerer);
    end
end
...
function level_monster_killed(monster, player, shot)
   ...
   if (w1 ~= nil and monster.index == w1.index) and (not gabriel_dead) then
      w1 = Monsters.new(banquet_l1.x, banquet_l1.y, 0, banquet_l1, _weaker_knight);
      w1.yaw = 180;
   end
   if (w2 ~= nil and monster.index == w2.index) and (not gabriel_dead) then
      w2 = Monsters.new(banquet_l1.x, banquet_l1.y, 0, banquet_l1, _weaker_knight);
      w2.yaw = 180;
   end
   if (mtype == _gabriel2) then
