...
respawn = {
   [_minor_dinobug] = 226, [_minor_raptor] = 102,
   [_future_bob] = 148, [_ranger] = 160,
   [_white_knight] = 154, [_cavebob] = 138,
   [_dimorph] = 302, [_trex] = 6 };
respawn2 = {
   [_ranger] = 9, [_dimorph] = 299, [_minor_dinobug] = 346 };
laps = 0;
prev_lap = 0;
lap_timer = 0;
lap_cum = 0;
...
function level_idle ()
    if not flint_exists then
        flintmobile = Monsters.new(flint_poly.x, flint_poly.y, 0, flint_poly, _throne);
    ...
    if in_vehicle then
        ...
        racetrack_radar(32);
        display_stats();
        if (Players[0].weapons.active ~= _weapon_wand) then
	        Players[0].weapons[_weapon_wand]:select();
        end
    ...
    if (resident_kills >= revolt_kills) and (not revolt) then
        ...
        revolt = true;
        MonsterTypes[_futurebob].enemies["player"] = true;
        MonsterTypes[_cavebob].enemies["player"] = true;
        MonsterTypes[_white_knight].enemies["player"] = true;
        MonsterTypes[_ranger].enemies["player"] = true;
        MonsterTypes[_dimorph].enemies["player"] = true;
        MonsterTypes[_trex].enemies["player"] = true;
        MonsterTypes[_maint_bob].enemies["player"] = true;
    end
    poly = Players[0].polygon;
    if poly.index ~= last_poly then
        last_poly = poly.index;
        if (poly.index == flint_poly.index) and (Players[0].items[_item_key] > 0) then
            flintmobile:damage(1001, "suffocation");
            Players[0].items[_item_wand] = 1;
            Players[0].items[_item_key] = 0;
            Players[0]:play_sound(263, 1);
            in_vehicle = true;
            milestone_counter = 1;
            mpoly = Polygons[milestone_polys[milestone_counter]];
	        Items.new(mpoly.x, mpoly.y, 0, mpoly, _item_magic_scroll);
	        Music.fade(60/1000);
...
function Triggers.player_killed(player, killer, action, projectile)
    ...
    y = player.y;
    facing = player.yaw;
    local car = Monsters.new(x, y, 0, poly, _throne);
    car.yaw = facing;
end

function level_got_item(type, player)
    if type == _item_key then
        Players[0]:play_sound(254, 1);
    end
    if type == _item_magic_scroll then
...
function level_monster_killed(victim, victor, projectile)
    ...
    if (victor == Players[0]) then
        ...
        if (mclass == _class_friend) or (mclass == _class_raptor) then
...
function racetrack_radar(range)
    ...
    if (Players[0].dead) then
        ...
        for n in Items() do
            if (n.type == _item_magic_scroll) then
