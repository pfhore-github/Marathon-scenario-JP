...
function level_idle ()
    ...
    if merlin_timer > 0 then
        ...
        -- 字幕
        if( merlin_timer < 4*30) then
            Players[0].overlays[0].text = "私の変化は完了しました。"
        elseif( merlin_timer < 10 * 30) then
            Players[0].overlays[0].text = "私は自由で、自由に土地を旅することができます。"
        elseif( merlin_timer < 13 * 30) then 
            Players[0].overlays[0].text = "そして、久しぶりに…とても長らく…"
        elseif( merlin_timer < 17 * 30) then
            Players[0].overlays[0].text = "最後に、愛するヘーゼルに会うでしょう。"
        elseif( merlin_timer < 20 * 30) then
            Players[0].overlays[0].text = "心から彼女が私のメッセージを受け取ったことを願っています。 "
        elseif( merlin_timer < 22 * 30) then
            Players[0].overlays[0].text = "すぐにわかります。"
        elseif( merlin_timer < 27 * 30) then
            Players[0].overlays[0].text = "私の友人のあなたにとって、私の謝意は永遠です。"
        elseif( merlin_timer < 30 * 30) then
            Players[0].overlays[0].text = "これを受け取ってください。それはあなたをあなた自身の時間に戻します。"
        elseif( merlin_timer < 38 * 30) then
            Players[0].overlays[0].text = "あなたは、モルガナの黒魔術の邪悪な支配から人類を解放したヒーローとして戻ります"
        elseif( merlin_timer < 45 * 30) then
            Players[0].overlays[0].text = "常に誇りを持ってこれらの時代を振り返ってください。 そして覚えておいてください、"
        elseif( merlin_timer < 48 * 30) then
            Players[0].overlays[0].text = "人間が忘れなければならないのはそのの運命だからです。"
        elseif( merlin_timer < 52 * 30) then
            Players[0].overlays[0].text = "悪が本当に実際に破壊されることは決してなく、"
        elseif( merlin_timer < 56 * 30) then
            Players[0].overlays[0].text = "ただ休眠状態に置かれ、機会を待っていることを理解してください。"
        elseif( merlin_timer < 59 * 30) then
            Players[0].overlays[0].text = "本当の騎士道精神を持ち続け、"
        elseif( merlin_timer < 63 * 30) then
            Players[0].overlays[0].text = "そして暗黒の種子から発生する悪と戦い続けてください。"
        elseif( merlin_timer < 70 * 30) then
            Players[0].overlays[0].text = "真実に生きてください。そうすれば、エクスカリバーの力はいつもあなたの中で輝くでしょう。"
        elseif( merlin_timer < 76 * 30) then
            Players[0].overlays[0].text = "勇敢な騎士、それは名誉です。"
        end
