...
function level_init(rs)
    ...
    if rs then
        if (Players[0].weapons.current == _weapon_wand) then
            activate_bike();
        end
      --[[
...
function record_monsters()
    ...
    for g in Monsters() do 
        if (g ~= player) then
            mtype = g.type;
            if ((mtype == _minor_gat) or (mtype == _major_gat) or
       (mtype == _hover_gat) or (mtype == _bike_cop) or (mtype == _laser_turret) or
(mtype == _minor_spam) or (mtype == _major_spam)) then
                local m = {};
                m.mtype = mtype;
                m.polygon = g.polygon;
                m.facing = g.facing;
...
--]]
    ...
    Players[0].overlays[5].text = string.format('速度: %d mph', math.abs(speed));
    if speed < 0 then
