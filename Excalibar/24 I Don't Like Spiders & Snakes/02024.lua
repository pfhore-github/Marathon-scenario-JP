...
enter_to[617] = function()
    Players[0]:teleport_to_level(25);
end

enter_to[603] = enter_to[617]
...
function bell_time()
    ...
    Players[0].overlays[0].text = string.format(" タワー %02i:%02i",hour,min);
    ...
    if bell_timer > bell_wait then
        ...
        if bell_cnt >= bell_rings then  --[[ Finished chimes for hour --]]
            ...
            if (bell_rings == 12) then
                ...
                Monsters.new(Polygons[564].x, Polygons[564].y, 0, 564, _black_knight);
                Monsters.new(Polygons[186].x, Polygons[186].y, 0, 186, _fire_beast);
                Monsters.new(Polygons[52].x, Polygons[52].y, 0, 52, _spider);
                Monsters.new(Polygons[50].x, Polygons[50].y, 0, 50, _spider);
