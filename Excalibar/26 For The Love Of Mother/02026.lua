...
function level_init (rs)
    ...
    grail = find_monster(_grail);
    mordred = find_monster(_mordred);
    if rs then
...
enter_to[wand_poly] = function()
    if wand_chamber_active and (not wand_poly_active) then
        if has_wand then
    ...
    if wand_poly_active then
        wand_poly_active = false;
        Players[0].items[_item_magic_scroll] = math.max(0, Players[0].items[_item_magic_scroll] - 10);
        Players[0].items[_item_wand] = Players[0].items[_item_wand] + 1;
    end
end
enter_to[grail_trigger_poly] = function()
    if not grail_free then
        free_grail();
    end
end

enter_to[mordred_poly] = function()
...
function level_idle ()   
   ...
   --]]
    ...
    if (not mordred_dead) and (mordred == -1) then
        if not found_item(_item_sap_card) then
...
function mordred_dies()
    ...
    Items.new(card_poly.x, card_poly.y, 512, card_poly, _item_sap_card);
...
function level_monster_killed(monster, player, shot)
    ...
    if (mtype == _mordred) and (not mordred_dead) then
        mordred_dies();
    end
end
