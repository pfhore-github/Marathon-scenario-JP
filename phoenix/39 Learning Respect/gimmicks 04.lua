...
function Triggers.idle()
   for p in Players() do
      if p.z < -12 and not p.dead then
         p:damage(15,"suffocation")
      end
   end
   for m in Monsters() do
      if m.z < -12 and not m.dead and not m.player then
         m:damage(45,"suffocation")
      end
      if m.z < -8.00 and not m.player then
